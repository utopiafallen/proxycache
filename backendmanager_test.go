...
func newTestBM(t *testing.T, cfg []map[string]any) *BackendManager {
	t.Helper()
	oldMetaDir := MetaDir
	MetaDir = t.TempDir()
	t.Cleanup(func() { MetaDir = oldMetaDir })
...
func TestBackendCacheDirPerBackend(t *testing.T) {
	...
	be1 := SanitizeBackendDir("10.0.0.1:8000")
	be2 := SanitizeBackendDir("10.0.0.2:8000")
	keys := bm.Keys()
...
func TestBackendCacheDirMutualExclusivity(t *testing.T) {
	assertPanic(t, func() {
		NewBackendManager([]map[string]any{{"url": "http://10.0.0.1:8000", "cache_dir": "/mnt/cache", "agent_port": 8082}})
	})
	assertPanic(t, func() {
		NewBackendManager([]map[string]any{{"url": "http://10.0.0.1:8000"}})
	})
}

func bmWithModels(t *testing.T, models ...*DiscoveredModel) *BackendManager {
	t.Helper()
	bm := &BackendManager{
...
func dm(name string, nCtx int, backends ...string) *DiscoveredModel {
	return &DiscoveredModel{Name: name, NCtx: nCtx, Backends: backends, BackendNCTX: map[string]int{}}
}
...
func TestGetModelNCtx(t *testing.T) {
	...
	if got := bm.GetModelNCtx("missing"); got != DefaultNCtx {
		t.Errorf("GetModelNCtx(missing) = %d, want %d", got, DefaultNCtx)
	}
...
func TestGetBackendNCtx(t *testing.T) {
	...
	if got := bm.GetBackendNCtx("model-a", "be3"); got != DefaultNCtx {
		t.Errorf("be3 = %d, want %d", got, DefaultNCtx)
	}
	if got := bm.GetBackendNCtx("unknown-model", "be1"); got != DefaultNCtx {
		t.Errorf("unknown-model = %d, want %d", got, DefaultNCtx)
	}
}
...
func TestDiscoverModelsCtxNotInArgs(t *testing.T) {
	...
	if len(models) != 1 || models[0].NCtx != DefaultNCtx {
		t.Errorf("got %+v, want n_ctx=%d", models, DefaultNCtx)
	}
}
...
func TestDiscoverModelsNonRouterMetaNull(t *testing.T) {
	...
	if len(models) != 1 || models[0].Name != "model-z" || models[0].NCtx != DefaultNCtx {
		t.Errorf("got %+v, want n_ctx=%d", models, DefaultNCtx)
	}
}
...
func TestBackendCacheDeleteViaLocal(t *testing.T) {
	...
	be := SanitizeBackendDir("10.0.0.1:8000")
...
func TestBackendCacheDeleteViaAgent(t *testing.T) {
	...
	be := SanitizeBackendDir(hostPort)
	if !bm.CacheDelete(be, "test_key") {
