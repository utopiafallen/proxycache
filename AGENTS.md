...
- **Slot acquire retry**: retries forever until a slot frees (growing backoff `SLOT_ACQUIRE_RETRY_BASE_SECONDS` × attempt, default base 5s). After each backoff the candidate set is re-derived from live model discovery (`GetDiscoveredModels`), so backends that come online mid-wait (liveness loop) join the pool and backends that go offline drop out. No 503 on slot exhaustion — a request only aborts on client disconnect / context cancellation.
