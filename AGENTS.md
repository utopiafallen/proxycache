...
- **Go port build**: `./build-proxycache.sh` (repo root) builds `go-proxycache/` with Windows Go from WSL and outputs `proxycache.exe` to the repo root. `./build-cache-agent.sh` does the same for the cache agent.
